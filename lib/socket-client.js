...
let connectUrl = 'https://api.dev.lotun.io'
if (process.env.NODE_ENV === 'local') {
  connectUrl = 'http://api.lotun.local:3000'
}
  ...
  */
  ...
  socket.on('error', function(err) {
    if (err === 'not authorized' && lastError != err) {
      ...
      console.log(`https://dashboard.dev.lotun.io/create-device?token=${encodeURIComponent(options.token)}&name=${encodeURIComponent(options.hostname)}`)
    }
